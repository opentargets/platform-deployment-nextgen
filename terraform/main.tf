...
resource "google_container_cluster" "cluster" {
  ...
  resource_labels = merge(var.base_labels, var.cluster_labels)
...
resource "google_container_node_pool" "pools" {
  ...
  node_config {
    ...
    labels          = merge(var.base_labels, var.cluster_labels, each.value.labels)
    tags            = ["cluster", "node"]
...
resource "google_container_node_pool" "databases_clickhouse" {
  ...
  node_config {
    ...
    image_type      = "COS_CONTAINERD"
    service_account = google_service_account.node.email
    labels          = merge(var.base_labels, var.cluster_labels, { pool = "clickhouse" })
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      parameters:
        ...
        provisioned-iops-on-create: "${var.cluster_disk_iops}"
        provisioned-throughput-on-create: "${var.cluster_disk_throughput}Mi"
...
resource "google_container_node_pool" "pools_opensearch" {
  ...
  node_config {
    ...
    image_type      = "COS_CONTAINERD"
    service_account = google_service_account.node.email
    labels          = merge(var.base_labels, var.cluster_labels, { pool = "opensearch" })
